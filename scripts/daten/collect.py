...
Findet und startet alle aktiven Sammler (collect_*.py im scripts/daten-Ordner).
Nach der Sammlung wird die key-metrics.md neu erzeugt, damit /prime immer
frische Zahlen lädt.

Nutzung:
    python scripts/daten/collect.py                          # Alle Sammler
    python scripts/daten/collect.py --sources youtube,instagram  # Nur bestimmte
    python scripts/daten/collect.py --date 2026-02-20         # Datum überschreiben
"""
...
def discover_collectors():
    """Alle collect_*.py Dateien im scripts/daten-Ordner finden."""
    collectors = {}
...
def main():
    ...
    if not available:
        print(f"[{timestamp}] Keine Sammler gefunden.", file=sys.stderr)
        sys.exit(1)
