#!/bin/bash
# ─────────────────────────────────────────────────────────────
# Jarvis Täglicher Auto-Lauf
# Wird von launchd aufgerufen (siehe ~/Library/LaunchAgents/com.jarvis.daily.plist)
...
python3 "$PROJECT_DIR/scripts/jarvis/run.py" --once --count 2 >> "$LOG_FILE" 2>&1
